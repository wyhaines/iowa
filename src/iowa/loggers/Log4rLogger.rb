      ...
      def check_errors
        if Iowa.config.has_key? Clogging
          if Iowa.config[Clogging].has_key? Cbasedir
            raise "The logging basedir (#{File.join(Iowa::Application.root_directory, Iowa.config[Clogging][Cbasedir])}) must be a directory." unless FileTest.directory? File.join(Iowa::Application.root_directory, Iowa.config[Clogging][Cbasedir])
            raise "The logging basedir (#{File.join(Iowa::Application.root_directory, Iowa.config[Clogging][Cbasedir])}) must be writeable." unless FileTest.writable? File.join(Iowa::Application.root_directory,Iowa.config[Clogging][Cbasedir])
          end
