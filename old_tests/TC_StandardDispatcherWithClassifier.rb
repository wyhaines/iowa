...
require 'iowa/dispatchers/StandardDispatcherWithClassifier'
require 'benchmark'
...
class TC_StandardDispatcherWithClassifier < Test::Unit::TestCase
	...
	def test_a_empty_struct
		...
		assert_nothing_raised("Failure parsing an empty ruleset") do
			rs = Iowa::Dispatchers::StandardDispatcherWithClassifier::RuleSet.new(rule_struct)
		end
	end
...
EMAPFILE
		rqs = [
			Request.new('/funds/portfolio.html','foo.com',{}),
			Request.new('/funds/blah/blarg/index.html','foo.com',{}),
			Request.new('/INDEX.HTM','www.foo.com',{}),
			Request.new('/INDEX.HTM','fluffy.poodlelovers.com',{}),
		...
		puts mf.inspect
		#assert_nothing_raised("Failure while parsing ruleset.") do
			disp = Iowa::Dispatchers::StandardDispatcherWithClassifier.new(mf)
			dd = disp.process(rqs[0])
			processing rqso[0]
			puts "**  #{dd.inspect}"
			assert(dd.component == 'Default',"CapIndex1: Processing with result from map failed.")

			dd = disp.process(rqs[1])
			processing rqso[1]
			puts "  #{dd.inspect}"
			assert(dd.component == 'Default',"CapIndex2: Processing with result from map failed.")
