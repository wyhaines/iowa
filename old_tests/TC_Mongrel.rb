...
if IWATestSupport.run_modes.include?(:mongrel)
	class TC_Mongrel < Test::Unit::TestCase

		@@testdir = IWATestSupport.test_dir(__FILE__)
		def setup
			Dir.chdir(@@testdir)
			IWATestSupport.announce(:mongrelmode,"Mongrel Run Mode")
	
			ruby = File.join(::Config::CONFIG['bindir'],::Config::CONFIG['ruby_install_name'])
			ruby << ::Config::CONFIG['EXEEXT']
	
			assert_nothing_raised("setup failed") do
				@iowa_pid = IWATestSupport::create_process(:dir => 'TC_Mongrel/iowa',
					:cmd => ["#{ruby} -I../../../src app.rb -r mongrel"])
				sleep 1
			end
		end
