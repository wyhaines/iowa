...
class TC_NoSubclass < Test::Unit::TestCase
	...
	def setup
		Dir.chdir(@@testdir)
		IWATestSupport.announce(:nosubclass,"App With No Subclass")

		ruby = File.join(::Config::CONFIG['bindir'],::Config::CONFIG['ruby_install_name'])
		ruby << ::Config::CONFIG['EXEEXT']

		assert_nothing_raised("setup failed") do
			@iowa_pid = IWATestSupport::create_process(:dir => 'TC_NoSubclass/iowa',
				:cmd => [ruby,'-I../../../src','app.rb'])
			sleep 1
			@wb_pid = IWATestSupport::create_process(:dir => 'TC_NoSubclass',
